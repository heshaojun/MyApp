...
const handleByMeList = ref([]);
//拉取数据
const onload_ = () => {
  loading.value = true;
  for (let i = 0; i < 6; i++) {
    handleByMeList.value.push(Math.random());
  }
  loading.value = false;
  ...
  <div class="handle-by-me">
    <van-pull-refresh v-model="refreshing" @refresh="onRefresh_">
      <van-list v-model:loading="loading" @load="onload_" finished-text="没有更多数据了">
        <van-cell v-for="item in handleByMeList" :key="item">
          <template #default>
            <div
              ...
              @click="
                router.push({
                  path: '/workDetail',
                  query: { workId: item, workType: 'handleByMe' },
                })
              "
            >
              <div style="flex: 1; margin-right: 0.5rem">
                ...
                >
                  <div>任务名称</div>
                  <div style="font-size: var(--van-font-size-sm); margin-right: 1rem">
                    任务状态
                  </div>
                </div>
                <div style="color: var(--van-gray-5); font-size: var(--van-font-size-sm)">
                  任务内容摘要任务内容摘要任务内容摘要任务内容摘要任务内容摘要任务内容摘要...
                </div>
                <div
                  style="
                    display: flex;
                  ...
                  "
                >
                  <div style="width: 50%">创建人：某某某某</div>
