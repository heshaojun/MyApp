...
const newWorkFormRules = reactive({
  workName: [
    {required: true, message: "请填写工作名称"}
  ],
  workType: [
    {required: true, message: "请选择工作类型"}
  ],
  workStation: [
    {required: true, message: "请选择工作的场站"}
  ],
  workHandler: [
    {required: true, message: "请指定工作执行人"}
  ],
  workChecker: [
    {required: true, message: "请指定工作验收人"}
  ],
  startTimeInPlan: [
    {required: true, message: "请选择工作计划开始时间"}
  ],
  endTimeInPlan: [
    {required: true, message: "请选择工作计划完成时间"}
  ],
  workContent: [
...
<template>
  <div class="add-new-work">
    <van-nav-bar title="创建新工作" right-text="取消" @click-right="router.back()" :placeholder="true"
                 :safe-area-inset-top="true"/>
    <van-form @submit="submitAddNewWorkForm_">
      <van-field name="workName" v-model="newWorkForm.workName" label="工作名称" placeholder="请填写工作名称"
                 :rules="newWorkFormRules.workName" required/>
      <van-field name="workType" label="工作类型" required :rules="newWorkFormRules.workType">
        <template #input>
      ...
      <van-field name="workStation" label="工作场站" placeholder="请选择工作场站" readonly is-link required
                 @click="showStationOption = true" v-model="newWorkForm.workStationName"
                 :rules="newWorkFormRules.workStation"/>
      <van-field name="workHandler" label="执行人" placeholder="请选择工作执行人" readonly is-link required
                 @click="showHandlerOption = true" v-model="newWorkForm.workHandlerName"
                 :rules="newWorkFormRules.workHandler"/>
      <van-field name="workChecker" label="验收人" placeholder="请选择工作验收人" readonly is-link required
                 @click="showCheckerOption = true" v-model="newWorkForm.workCheckerName"
                 :rules="newWorkFormRules.workChecker"/>
      <van-field name="startTimeInPlan" label="开始时间" placeholder="请选择计划开始时间" readonly is-link required
                 @click="showStartTimePicker = true" v-model="newWorkForm.startTimeInPlan"
                 :rules="newWorkFormRules.startTimeInPlan"/>
      <van-field name="endTimeInPlan" label="截止时间" placeholder="请选择计划截止时间" readonly is-link required
                 @click=" endTimeClient_" v-model="newWorkForm.endTimeInPlan"
                 :rules="newWorkFormRules.endTimeInPlan"/>
      <van-field name="workContent" label="工作内容" placeholder="请输入工作内容" v-model="newWorkForm.workContent" required
                 type="textarea" rows="5"
                 maxlength="500" show-word-limit border :rules="newWorkFormRules.workContent"/>
