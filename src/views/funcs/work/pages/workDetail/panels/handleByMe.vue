...
const optionActions = ref([
  { text: "开始处理", mark: "delete", disabled: false, icon: "close" },
  { text: "提交验收", mark: "delete", disabled: false, icon: "close" },
]);
...
onMounted(() => {
  ...
  if (workInfo.workState === "waitForHandling") {
    optionActions.value = [
      { text: "开始处理", mark: "delete", disabled: false, icon: "add-o" },
      { text: "提交验收", mark: "delete", disabled: true, icon: "certificate" },
    ];
  } else {
    optionActions.value = [
      { text: "开始处理", mark: "delete", disabled: true, icon: "add-o" },
      { text: "提交验收", mark: "delete", disabled: false, icon: "certificate" },
    ];
  }
});
</script>
<template>
  <div class="handle-by-me">
    <van-nav-bar title="我处理的工作" left-arrow left-text="返回" @click-left="router.back()" :safe-area-inset-top="true">
      <template #right>
        <van-popover v-model:show="showOption" :actions="optionActions" placement="bottom-end" style="width: 8rem">
          <template #reference>
