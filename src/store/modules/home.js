...
const mutations = {
    ...
    SET_APPEOVE_LIST: (state, approveList) => state.approveList = approveList,
...
const actions = {
    ...
    fetchWorkList: ({ commit }) => {
        commit("SET_WORK_LIST", [{ title: "某某变电站检修", state: "正在执行", color: "red" }, { title: "某某变电站检修", state: "等待验收", color: "red" }]);
    },
    //获取首页新闻列表
    fetchNewsList: ({ commit }) => {
        commit("SET_NEWS_LIST", [{ title: "新闻标题", imgs: ["照片1"], source: "电网头条", publisTime: "发布时间" }, { title: "新闻标题", imgs: ["照片1", "照片2", "照片3"], source: "电网头条", publisTime: "发布时间" }, { title: "新闻标题", imgs: [], source: "电网头条", publisTime: "发布时间" }])
    },
    //获取审批信息列表
    fetchApproveList: ({ commit }) => {
        commit("SET_APPEOVE_LIST", [{ title: "某某变电站检修", state: "正在执行", color: "red" }, { title: "某某变电站检修", state: "等待验收", color: "red" }]);
    },
    //获取用户定位信息
    fetchLocationInfo: ({ commit }) => {
        commit("SET_LOCATION_INFO", { location: "xx变电站", state: "inside" })
    }
}
